...
class ServerConfig {
  constructor (invokedFromCLI, store) {
    this.app = express()
    this.store = store
  ...
  _configureStatics({assets, configRoot}) {
    assets.forEach(({urlPath, directory}) => {
      this.app.use(urlPath, express.static(path.resolve(configRoot, directory)))
    })
  }
  ...
  _loadInitialState({initialState = {}}) {
    this.store.dispatch({type: 'LOAD_INITIAL_STATE', routeData: initialState})
  }

  _configureManagementInterface () {
    const { vhostApp } = new ManagementInterfaceConfig(this.store)
  ...
  stateProviderMiddleware (req, res, next) {
    let { path } = req
    let { routeData } = this.store.getState()
  ...
  stateUpdaterMiddleware (req, res, next) {
    ...
    res.recordResponse = (data) => {
      ...
      this.store.dispatch({type: 'SET_ROUTE_DATA', routeData})
  ...
  async modifierMiddleware (req, res, next) {
    let { path, method } = req
    let { modifiers } = this.store.getState()
